  ...
  return (
    <div className="app-shell">
      {shellType === "public" ? (
        <>
          {view === "login" || view === "register" || view === "forgot" ? null : <PublicNav />}
          <main>{renderPublicView(view, handleLoginSuccess)}</main>
        </>
      ) : shellType === "donor" ? (
        <DashboardShell title="Donor Portal" subtitle="Donation activity, appointments, and urgent requests" links={donorLinks} active={view} user={currentUser} onLogout={handleLogout}>
          {renderDonorView(view)}
        </DashboardShell>
      ) : (
        <DashboardShell title="Facility Admin" subtitle="Al-Shifa Hospital - Gaza City" links={adminLinks} active={view} user={currentUser} onLogout={handleLogout}>
          {renderAdminView(view)}
        </DashboardShell>
      )}
    </div>
...
function renderPublicView(view: View, onLoginSuccess: (user: AuthUser) => void) {
  if (view === "terms") return <LegalPage kind="terms" />;
  if (view === "privacy") return <LegalPage kind="privacy" />;
  if (view === "login") return <Login onSuccess={onLoginSuccess} />;
  if (view === "register") return <Register onSuccess={onLoginSuccess} />;
  if (view === "forgot") return <ForgotPassword />;
  return <PublicWebsite />;
}
...
function Login({ onSuccess }: { onSuccess: (user: AuthUser) => void }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    window.location.hash = result.user.role === "admin" || result.user.role === "facility" ? "#admin" : "#donor";
  }
  ...
  return (
    ...
    >
      <Input label="Email" placeholder="name@example.com" type="email" value={email} onChange={(e) => setEmail((e.target as HTMLInputElement).value)} />
      <Input label="Password" placeholder="Enter password" type="password" value={password} onChange={(e) => setPassword((e.target as HTMLInputElement).value)} />
...
function Register({ onSuccess }: { onSuccess: (user: AuthUser) => void }) {
  const [role, setRole] = useState<"donor" | "facility">("donor");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    onSuccess(result.user);
    window.location.hash = role === "facility" ? "#admin" : "#donor";
  }
  ...
  return (
    ...
    >
      ...
      {role === "donor" ? (
        <>
          <div className="form-two">
            <Input label="Full Name" placeholder="John Doe" value={fullName} onChange={(e) => setFullName((e.target as HTMLInputElement).value)} />
            <Input label="Email" placeholder="john@example.com" type="email" value={email} onChange={(e) => setEmail((e.target as HTMLInputElement).value)} />
          </div>
          <div className="form-two">
            <Input label="Password" placeholder="********" type="password" value={password} onChange={(e) => setPassword((e.target as HTMLInputElement).value)} />
            <Input label="Confirm Password" placeholder="********" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword((e.target as HTMLInputElement).value)} />
          </div>
          <div className="form-two">
            <Input label="Phone" placeholder="+970..." value={phone} onChange={(e) => setPhone((e.target as HTMLInputElement).value)} />
            <Input label="City" placeholder="Gaza City" value={city} onChange={(e) => setCity((e.target as HTMLInputElement).value)} />
          </div>
          <div className="form-two">
            <SelectField label="Blood Type" options={["Select type", ...bloodTypeOptions]} value={bloodType} onChange={setBloodType} />
            <SelectField label="Gender (Optional)" options={["Select gender", "Male", "Female"]} value={gender} onChange={setGender} />
          </div>
          {error && <span className="field-error">{error}</span>}
          <button className="button button-primary full-width" type="submit" disabled={loading}>
        ...
        <>
          <div className="form-two">
            <Input label="Facility Name" placeholder="Al-Shifa Hospital" value={fullName} onChange={(e) => setFullName((e.target as HTMLInputElement).value)} />
            <Input label="City" placeholder="Gaza City" value={city} onChange={(e) => setCity((e.target as HTMLInputElement).value)} />
          </div>
          <Input label="Full Address" placeholder="Street, Neighborhood" />
          <div className="form-two">
            <Input label="Admin Full Name" placeholder="Jane Doe" />
            <Input label="Admin Email" placeholder="admin@hospital.com" type="email" value={email} onChange={(e) => setEmail((e.target as HTMLInputElement).value)} />
          </div>
          <div className="form-two">
            <Input label="Password" placeholder="********" type="password" value={password} onChange={(e) => setPassword((e.target as HTMLInputElement).value)} />
            <Input label="Confirm Password" placeholder="********" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword((e.target as HTMLInputElement).value)} />
          ...
          <div className="form-two">
            <Input label="Contact Phone" placeholder="+970..." value={phone} onChange={(e) => setPhone((e.target as HTMLInputElement).value)} />
            <Input label="Contact Person (Optional)" placeholder="Jane Doe" />
          </div>
          {error && <span className="field-error">{error}</span>}
          <button className="button button-primary full-width" type="submit" disabled={loading}>
...
function AuthPage({ eyebrow, title, subtitle, children, footer, onSubmit }: { eyebrow: string; title: string; subtitle: string; children: ReactNode; footer: ReactNode; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return (
    <section className="auth-screen">
      <BrandMark href="#top" label="BloodConnect Gaza" />
      <div className="auth-card">
...
function DashboardShell({ title, subtitle, links, active, children, user, onLogout }: { title: string; subtitle: string; links: NavItem[]; active: View; children: ReactNode; user: AuthUser | null; onLogout: () => void }) {
  const isAdmin = title.includes("Admin");
  const navActive = !isAdmin && (active === "facility-details" || active === "book-appointment") ? "donor-search" : active;
  const activeLabel = links.find((item) => item.href === navActive)?.label ?? (isAdmin ? "Overview" : "Dashboard");
  const donorChildTitle = active === "facility-details" ? "Facility Details" : active === "book-appointment" ? "Book Appointment" : activeLabel;
  ...
  return (
    <div className="dashboard-shell">
      <aside className="dashboard-sidebar">
        ...
        <button className="sidebar-exit" type="button" onClick={onLogout}><Icon name="logout" />Logout</button>
      </aside>
      <div className="dashboard-main">
        <header className="dashboard-topbar">
          <h1>{shellTitle}</h1>
